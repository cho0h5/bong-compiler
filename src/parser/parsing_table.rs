...
pub fn get_rule(state: usize, token: &Token) -> Option<TableElement> {
    ...
    if state == 3 {
        match token {
            ...
            Identifier(_, _) => return Some(Shift(9)),
            _ => return None,
        };
    }
    ...
    if state == 4 {
        match token {
            ...
            Identifier(_, _) => return Some(Reduce(3)),
            _ => return None,
        };
    }
    ...
    if state == 5 {
        match token {
            ...
            Identifier(_, _) => return Some(Reduce(4)),
            _ => return None,
        };
    }
    ...
    if state == 6 {
        match token {
            ...
            Identifier(_, _) => return Some(Reduce(5)),
            _ => return None,
        };
    }
    ...
    if state == 7 {
        match token {
            ...
            Identifier(_, _) => return Some(Reduce(6)),
            _ => return None,
        };
    }
    ...
    if state == 11 {
        match token {
            Identifier(_, _) => return Some(Reduce(8)),
            _ => return None,
        };
    }
    ...
    if state == 16 {
        match token {
            ...
            Identifier(_, _) => return Some(Shift(50)),
            ...
            If(_) => return Some(Shift(35)),
            While(_) => return Some(Shift(36)),
    ...
    if state == 19 {
        match token {
            Lbracket => return Some(Shift(10)),
            Star => return Some(Shift(11)),
            Identifier(_, _) => return Some(Shift(54)),
            _ => return None,
        };
    }
    ...
    if state == 20 {
        match token {
            Identifier(_, _) => return Some(Reduce(7)),
            _ => return None,
        };
    }
    ...
    if state == 31 {
        match token {
            Lbracket => return Some(Shift(10)),
            Star => return Some(Shift(11)),
            Identifier(_, _) => return Some(Shift(58)),
            _ => return None,
        };
    }
    ...
    if state == 32 {
        match token {
            ...
            Identifier(_, _) => return Some(Shift(73)),
    ...
    if state == 51 {
        match token {
            ...
            Identifier(_, _) => return Some(Shift(106)),
    ...
    if state == 90 {
        match token {
            ...
            Identifier(_, _) => return Some(Shift(150)),
    ...
    if state == 91 {
        match token {
            ...
            Identifier(_, _) => return Some(Shift(167)),
